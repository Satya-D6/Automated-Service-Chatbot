...
import { ArrowLeft, CheckCircle, XCircle, Clock, MessageSquare, Send, ExternalLink, User, Calendar, Tag, AlertTriangle, Paperclip, X } from 'lucide-react'
...
export default function TicketDetail({ ticket, admin, onBack, onResponded }) {
  ...
  setError('')
  try {
    // Step 1 — Upload attachment to ServiceNow if present
    if (attachment) {
      const formData = new FormData()
      formData.append('uploadFile', attachment)

      const uploadRes = await fetch(
        `/sn-api/api/now/attachment/file?table_name=u_university_request&table_sys_id=${ticket.sys_id}&file_name=${encodeURIComponent(attachment.name)}`,
        {
          method: 'POST',
          headers: {
    ...
    await sendResponse(ticket, {
      responseText: message + (attachment ? `\n\n📎 Attachment sent: ${attachment.name}` : ''),
      newState: action,
      adminName: admin.name,
    })

    setSent(true)
    setTimeout(() => { onResponded(); onBack() }, 2000)
  } catch (e) {
      ...
      <div style={s.layout}>
        ...
        <div style={s.leftCol}>
          ...
          {ticket.u_sentiment && ticket.u_sentiment !== 'neutral' && (
            <div style={{
              ...s.sentimentBox,
              background:   ticket.u_sentiment === 'urgent' ? 'rgba(239,68,68,0.08)'  : 'rgba(245,158,11,0.08)',
              borderColor:  ticket.u_sentiment === 'urgent' ? 'rgba(239,68,68,0.2)'   : 'rgba(245,158,11,0.2)',
            }}>
          ...
          <div style={s.card}>
            ...
            <div style={s.field}>
              ...
              <textarea
                ...
                placeholder={`Dear ${ticket.u_student_name || 'Student'},\n\nWrite your response here...\n\nThis will appear in their UniAssist chat portal automatically.`}
                rows={7}
                style={s.textarea}
              />
              <div style={{ fontSize:'0.72rem', color:'#525972', marginTop:4 }}>{message.length} characters</div>
              ...
              <div style={{ marginTop: 10 }}>
                ...
                {!attachment && (
                  <button
                    ...
                    style={{
                      ...
                      color: '#4f8ef7', fontSize: '0.8rem', cursor: 'pointer',
                    }}
                  >
                    <Paperclip size={14} /> Attach PDF File
                  </button>
                )}
                ...
                {attachment && (
                  <div style={{
                    ...
                    <span style={{ fontSize: '0.8rem', color: '#10b981', flex: 1 }}>
                      {attachment.name}
                    </span>
                    <span style={{ fontSize: '0.72rem', color: '#525972' }}>
                      {(attachment.size / 1024).toFixed(1)} KB
                    </span>
                    <button
                      type="button"
                      onClick={() => { setAttachment(null); fileInputRef.current.value = '' }}
            ...
            <p style={s.sendNote}>
              💡 This will update the ServiceNow ticket state to "<strong>{ACTIONS.find(a => a.key === action)?.label}</strong>" and notify the student in their UniAssist chat.
            </p>
          </div>
        </div>
      </div>
...
const s = {
  ...
  backBtn:      { display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.04)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 14px', color: '#8891a8', fontSize: '0.83rem' },
  snLink:       { display: 'flex', alignItems: 'center', gap: 6, color: '#4f8ef7', fontSize: '0.82rem', textDecoration: 'none', background: 'rgba(79,142,247,0.08)', border: '1px solid rgba(79,142,247,0.2)', borderRadius: 8, padding: '8px 14px' },
  ...
  rightCol:     {},
  card:         { background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 16, padding: '24px' },
  ...
  sentimentBox: { display: 'flex', alignItems: 'center', gap: 10, border: '1px solid', borderRadius: 10, padding: '12px 16px' },
  responseTitle:{ display: 'flex', alignItems: 'center', gap: 8, fontFamily: 'Syne, sans-serif', fontSize: '1rem', fontWeight: 700, color: '#e8eaf0', marginBottom: 6 },
  ...
  actionBtn:    { display: 'flex', flexDirection: 'column', alignItems: 'flex-start', background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border)', borderRadius: 10, padding: '10px 12px', textAlign: 'left', color: '#8891a8', transition: 'all 0.15s' },
  field:        { display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 14 },
  fieldLabel:   { fontSize: '0.78rem', color: '#8891a8', fontWeight: 500 },
  textarea:     { background: 'rgba(255,255,255,0.04)', border: '1px solid var(--border)', borderRadius: 10, padding: '12px 14px', color: '#e8eaf0', fontSize: '0.85rem', resize: 'vertical', lineHeight: 1.6, width: '100%' },
  errorBox:     { display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 8, padding: '10px 14px', color: '#ef4444', fontSize: '0.82rem', marginBottom: 12 },
  successBox:   { display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 8, padding: '10px 14px', color: '#10b981', fontSize: '0.82rem', marginBottom: 12 },
  sendBtn:      { width: '100%', background: 'linear-gradient(135deg, #4f8ef7, #7c5af0)', border: 'none', borderRadius: 10, padding: '12px 20px', color: '#fff', fontWeight: 600, fontSize: '0.9rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 12, transition: 'opacity 0.2s' },
